...
@Component({
  ...
  host: { 'data-slot': 'date-picker-trigger', class: 'contents' },
  template: `
    <button
      [id]="buttonId()"
      type="button"
...
export class HlmDatePickerTrigger implements BrnDatePickerTriggerBase {
  ...
  protected readonly _computedClass = computed(() =>
    hlm(
      'data-placeholder:text-muted-foreground border-transparent bg-input/50 hover:bg-input/50 h-9 w-full justify-between rounded-3xl px-3 font-normal shadow-none',
      this.userClass(),
    ),
  );
  ...
  public readonly variant = input<ButtonVariants['variant']>('ghost');
